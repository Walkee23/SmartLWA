            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle">
                    ...
                    <tbody>
                        <?php
                        try {
                            // Update query to include 'is_cleared' status
                            $sql = "SELECT u.user_id, u.unique_id, u.first_name, u.last_name, u.role, u.is_cleared,
                            ...
                            if ($stmt->rowCount() > 0) {
                                while ($row = $stmt->fetch()) {
                                    ...
                                    if ($row['is_cleared'] == 1) {
                                        ...
                                        $status_text = 'Officially Cleared';
                                    } elseif ($row['borrowed_count'] == 0 && $row['total_penalty'] == 0.00) {
                                        $status_badge = 'bg-primary';
                                        $status_text = 'Ready for Clearance';
                                    } else {
    ...
    <div class="modal fade" id="clearanceModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                        ...
                        <?php if ($clearanceData): ?>
                            <div class="card bg-light mb-3">
                                <div class="card-body p-3">
                                    ...
                                    <?php if ($clearanceData['already_cleared']): ?>
                                        <div class="alert alert-success mb-0 p-2">
                                            <i class="fas fa-check-double"></i> <strong>ALREADY CLEARED</strong><br>
                                            <small>User has been cleared for this term.</small>
                                        </div>
                                    <?php elseif ($clearanceData['is_eligible']): ?>
                ...
                <!-- Action Buttons -->
                <?php if ($clearanceData): ?>
                    <?php if ($clearanceData['is_eligible'] && !$clearanceData['already_cleared']): ?>
                        <!-- Case: Eligible and NOT yet cleared -->
                        <form action="/SmartLWA/app/controllers/CirculationController.php" method="POST">
                    ...
                    <?php elseif ($clearanceData['already_cleared']): ?>
                        <!-- Case: Already Cleared -->
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                            <button type="button" class="btn btn-success" disabled>Cleared Successfully</button>
                        </div>
                    <?php else: ?>
                        <!-- Case: Not Eligible -->
                        <div class="modal-footer">
